...
- **Calibrated outlier threshold.** The hard-coded distance cut is replaced by an in-distribution calibrated threshold: per category, the 95th percentile of leave-one-out kNN distances over the in-repo pool (so ~95% of the codebase's own code reads as "conforms"). Stored in a new `cfm_calibration` table. `conformity-backfill` now calibrates as its final step, and judgments report the threshold used plus whether it was calibrated; before backfill/calibration, verdicts fall back to a default and are flagged uncalibrated.
- New CLI commands: `conformity-backfill` (one-time: embed every committed function to build the descriptive pool, then calibrate), `conformity-calibrate` (recompute per-category thresholds from the stored vectors without re-embedding), and `conformity-judge [file]` (judge working-tree changes, or one file, against the pool).
- New MCP tool `judgeConformity({ filePath?, maxResults? })` (the 8th tool): per-function category, distance, verdict (`conforms`/`outlier`) with the threshold used and whether it was calibrated, and nearest existing functions, leading with the outliers; judges each committed function against the pool minus its own vector (no self-match).
...
### Known limitations
- Conformity category coverage is a single category (`function:body`) for now; class-shape and other categories are future work.
