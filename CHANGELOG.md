...
### Security
- Force the transitive `protobufjs` (pulled in by `@xenova/transformers` → `onnxruntime-web` → `onnx-proto`) up to a patched `^7.6.4` via an npm `overrides`, clearing all reported `protobufjs` advisories (`<=7.6.2`: code injection, prototype pollution, and several DoS vectors). `npm audit` now reports 0 vulnerabilities, and the embedding backend was verified to still load the ONNX model and produce correct 768-dim vectors with protobufjs 7. Note: npm `overrides` apply to this repo's own install/lockfile (and dependabot) but do not propagate when codebase-pkg is installed as a dependency — consumers wanting the same pin should add the override themselves until the upstream `onnxruntime-web` chain ships a fixed `protobufjs`.
...
- New CLI command **`conformity-target [--init] [--force]`**: a read-only view of the effective decision target and **per-axis migration progress** computed entirely from the stored `cfm_decisions` distributions (no re-parse, no re-embed) — per axis it shows the target value, whether it's the descriptive seed or a `conformity-target.json` override, whether it's enforced or guarded out, and the share of deciders already at target vs. left to migrate. `--init` writes a starter `conformity-target.json` seeded from the current descriptive modes for the enforced axes (refusing to clobber an existing file without `--force`); edit and commit it and both the judge and backfill honor it.
- **Conformity Judge (embedding novelty — the secondary lens).** A local, offline read on how *structurally* unusual new code is. It judges three kinds of code entity, each against a pool of its own kind: functions and methods (`function:body`), types — classes, interfaces, enums, and type aliases (`type:body`), and module-level constant/config declarations (`module:const`). Each entity's **whole body, lightly normalized** (comments stripped and whitespace collapsed, identifiers/literals kept) is embedded and measured by cosine distance against the committed pool of same-category entities. It is an instrument, not a linter or a gate. Runs fully local — a code-specific embedding model (`jinaai/jina-embeddings-v2-base-code` via `@xenova/transformers`, ~162 MB, 768-dimensional, downloaded once then in-process), with `Xenova/jina-embeddings-v2-small-en` and MiniLM as fallbacks; no external API and no token cost.
- Conformity data lives in a provisioned **pgvector Postgres** cold store, keyed by `<filePath>::<name>` node id: embedding vectors in `cfm_vectors` (HNSW cosine ANN index) with an in-memory hot cache doing the kNN, per-category thresholds in `cfm_calibration`, and per-entity decision facts in `cfm_decisions`. `init --docker` now provisions the Postgres/pgvector service in the same compose file alongside Neo4j, bootstraps the schema once Postgres is reachable, and prefetches the embedding model. The Neo4j graph schema is unchanged — no conformity data lives in the graph.
...
- New CLI commands: `conformity-backfill` (one-time: persist the per-entity decision facts AND embed every committed entity — functions, types, and module-level constants — then calibrate), `conformity-calibrate` (recompute per-category thresholds from the stored vectors without re-embedding), `conformity-target [--init] [--force]` (show the effective decision target + migration from the store, or seed `conformity-target.json`), and `conformity-judge [file]` (judge working-tree changes, or one file: decision flags primary + novelty secondary).
- New MCP tool `judgeConformity({ filePath?, maxResults? })` (the 8th tool): leads with the **style-conformity** report (per-function off-target decisions vs. the effective target — "uses `let`; target is `const`"), then the **embedding-novelty** report (per-entity category, distance, verdict with the threshold used and whether it was calibrated, and nearest same-kind entities; each committed entity judged against its category's pool minus its own vector, no self-match).
...
### Known limitations
- Decision/style conformity is **TypeScript-only** (ts-morph extraction): `.ts`/`.tsx` functions, methods, and arrows. The curated axes don't model bare statements or destructuring-pattern declarations, and an axis a function makes no decision on (its absence value) is never faulted. The embedding-novelty lens is a separate signal answering a different question.
